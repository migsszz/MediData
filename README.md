...
4. (Optional but recommended) Run [`supabase/expire-guest-data.sql`](supabase/expire-guest-data.sql) to schedule hourly cleanup of guest accounts older than 24h via `pg_cron` (hosted projects only).
